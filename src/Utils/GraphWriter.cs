...
public class GraphWriter
{
    ...
    /// We need to use this attribute for our SourceRange attribute.
    /// </summary>
    private const string RegionStartAttribute = "Source.Region_Start";

    private const string HierarchicalEdgeType = "Part_Of";
    ...
    public static void Save(string filename, Graph graph, string hierarchicalEdgeType = HierarchicalEdgeType)
    {
        XmlDocument doc = new();
        ...
        {
            Trace.TraceError($"Could not save graph to GXL file '{filename}' due to: {e.Message}.\n");
            throw;
        }
    }
                                       ...
                                       ref int edgeCount)
    {
        foreach (Node child in graph.Nodes())
        {
            ...
            if (parent != null)
            {
                ...
                AppendEdge(doc, parentXMLNode, $"E{edgeCount}", hierarchicalEdgeType, source, target);
                edgeCount++;
            }
        }
    }
